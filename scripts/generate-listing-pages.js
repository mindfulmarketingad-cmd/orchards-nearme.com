...
/*
 * Generates individual business detail pages under /find/[slug] for a
 * hand-picked set of featured listings. Each page shows the searchmap
 * with a single listing card (full hours, phone, multiple reviews) and
 * the map zoomed to that business's location.
 *
 * Source data: data/featured-listings.json, fetched from the Google
 * Places API "Place Details" endpoint (no photo fields requested, ever).
 * Run `node scripts/fetch-featured-listings.js` to refresh it, then this
 * script to regenerate the HTML.
 */
...
// Guess a category from Places "types" since Place Details doesn't return
// our own site taxonomy.
function guessCategory(types) {
  types = types || [];
  if (types.includes('garden_center') || types.includes('home_goods_store') || types.includes('florist')) return 'Garden Center';
  if (types.includes('farm')) return 'Farm';
  return 'Orchard';
}
...
function generatePage(place) {
  const name = place.displayName ? place.displayName.text : 'Listing';
  const comps = place.addressComponents || [];
  const cityComp = getComponent(comps, 'locality') || getComponent(comps, 'administrative_area_level_3');
  const stateComp = getComponent(comps, 'administrative_area_level_1');
  const city = cityComp ? cityComp.longText : '';
  const state = stateComp ? stateComp.longText : '';
  ...
  const phoneLink = phone
    ? `<a href="${telHref(phone)}">Call ${escapeHtml(phone)}</a>`
    : '';
          ...
          <div class="cards">
            <article class="card listing-detail-card">
              ...
              <section class="listing-section">
                <h2>Hours</h2>
                ${hoursTableHtml(place.regularOpeningHours)}
              </section>

              <section class="listing-section">
                <h2>Reviews</h2>
                ${reviewsHtml(place.reviews)}
              </section>
...
const dataPath = path.join(__dirname, '..', 'data', 'featured-listings.json');
...
for (const place of places) {
  if (place.error) {
    console.log('Skipping (fetch error):', place.name);
    continue;
  }
  const name = place.displayName ? place.displayName.text : 'listing';
  const slug = slugify(name);
  const html = generatePage(place);
  fs.writeFileSync(path.join(findDir, `${slug}.html`), html, 'utf8');
  slugs.push(slug);
  console.log('Generated:', `${slug}.html`);
}

fs.writeFileSync(path.join(__dirname, '..', 'data', 'featured-listing-slugs.json'), JSON.stringify(slugs, null, 2));
console.log(`\nDone. Generated ${slugs.length} listing pages.`);
